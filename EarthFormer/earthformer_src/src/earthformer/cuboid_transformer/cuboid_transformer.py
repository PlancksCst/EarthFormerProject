...
class CuboidSelfAttentionLayer(nn.Module):
    ...
    def forward(self, x, global_vectors=None):
        ...
        if self.use_global_vector:
            ...
            try:
                autocast_enabled_local = torch.is_autocast_enabled()
            except Exception:
                autocast_enabled_local = False

            if autocast_enabled_local:
            ...
            # Compute global-vector attention in full precision when AMP/autocast
            # is enabled to avoid FP16 overflow / NaNs during softmax and matmuls.
            try:
